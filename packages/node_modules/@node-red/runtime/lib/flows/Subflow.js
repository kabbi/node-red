    ...
    start(diff) {
        ...
        if (this.subflowDef.in) {
            subflowInstanceConfig.wires = this.subflowDef.in.map(function(n) {
              return n.wires.map(function(w) { return { node: self.node_map[w.id].id, port: w.port || 0 }; });
            });
            subflowInstanceConfig._originalWires = clone(subflowInstanceConfig.wires);
        }

        var numInputs = this.subflowDef.in ? this.subflowDef.in.length : 1;
        this.node = new Node(subflowInstanceConfig);
        this.node.on("input", function(msg, send, done, port) {
          let result = new Array(numInputs).fill();
          result[port] = msg;
          this.send(result);
        });
        ...
        if (this.subflowDef.status) {
            ...
            var wires = this.subflowDef.status.wires;
            for (var j=0;j<wires.length;j++) {
                if (wires[j].id === this.subflowDef.id) {
                    // A subflow input wired straight to a subflow output
                    subflowInstanceConfig.wires[wires[j].port].push({ node: subflowStatusId, port: 0 });
                    this.node._updateWires(subflowInstanceConfig.wires);
                } else {
                    var node = self.node_map[wires[j].id];
                    if (!node._originalWires) {
                    ...
                    node.wires[wires[j].port].push({ node: subflowStatusId, port: 0 });
                }
            }
        }
        super.start(diff);
            ...
            if (node.wires) {
                ...
                for (var j=0;j<outputs.length;j++) {
                    ...
                    for (var k=0;k<wires.length;k++) {
                        if (nodeMap[outputs[j][k].node]) {
                            outputs[j][k].node = nodeMap[outputs[j][k].node].id
                        } else {
                            outputs[j][k] = null;
                        }
                    }
