            ...
            for (var j=0;j<wires.length;j++) {
                ...
                if (w.target.type != "subflow") {
                    if (w.sourcePort < node.wires.length) {
                        node.wires[w.sourcePort].push({ node: w.target.id, port: w.targetPort });
                    }
                }
            }
        ...
        n.out.forEach(function(p,c) {
            ...
            for (i=0;i<wires.length;i++) {
                if (wires[i].source.type != "subflow") {
                    ...
                    nOut.wires.push({id:n.id,port:wires[i].source.i})
                }
            }
            node.out.push(nOut);
        });
            ...
            links.forEach(function(d) {
                if (d.target === n.status) {
                    if (d.source.type != "subflow") {
                        ...
                        node.status.wires.push({id:n.id, port:d.source.i})
                    }
                }
            });
        }
            ...
            if (n.wires) {
                for (var w1=0;w1<n.wires.length;w1++) {
                    ...
                    for (var w2=0;w2<wires.length;w2++) {
                        if (node_map.hasOwnProperty(wires[w2].node)) {
                            if (n.z === node_map[wires[w2].node].z) {
                                var link = {source:n,sourcePort:w1,target:node_map[wires[w2].node],targetPort:wires[w2].port};
                                addLink(link);
                                new_links.push(link);
                            } else {
                                console.log("Warning: dropping link that crosses tabs:",n.id,"->",node_map[wires[w2].node].id);
                            }
                        }
                    }
                }
        ...
        for (i=0;i<new_subflows.length;i++) {
            ...
            n.out.forEach(function(output) {
                output.wires.forEach(function(wire) {
                    ...
                    if (subflow_map[wire.id] && subflow_map[wire.id].id == n.id) {
                        link = {source:n.in[wire.port], sourcePort:0,target:output,targetPort:0};
                    } else {
            ...
            if (n.status) {
                n.status.wires.forEach(function(wire) {
                    var link;
                    if (subflow_map[wire.id] && subflow_map[wire.id].id == n.id) {
                        link = {source:n.in[wire.port], sourcePort:0,target:n.status,targetPort:0};
                    } else {
