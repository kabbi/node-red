        ...
        if (mouse_mode == RED.state.IMPORT_DRAGGING) {
            ...
            if (activeSpliceLink) {
                ...
                var link1 = {
                    ...
                    sourcePort:spliceLink.sourcePort,
                    target: movingSet.get(0).n,
                    targetPort: spliceLink.targetPort // FIXME: Is this correct?
                };
                var link2 = {
                    source:movingSet.get(0).n,
                    sourcePort:0,
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                inGroup.append("g").attr('transform','translate(35,15)').append("rect").attr("class","red-ui-flow-port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                    .on("mousedown", function(d,i){portMouseDown(d,PORT_TYPE_OUTPUT,0);} )
                    .on("touchstart", function(d,i){portMouseDown(d,PORT_TYPE_OUTPUT,0);d3.event.preventDefault();} )
                    .on("mouseup", function(d,i){portMouseUp(d,PORT_TYPE_OUTPUT,0);})
                    .on("touchend",function(d,i){portMouseUp(d,PORT_TYPE_OUTPUT,0);d3.event.preventDefault();} )
                ...
                inGroup.append("svg:text").attr("class","red-ui-flow-port-label").attr("x",20).attr("y",12).style("font-size","10px").text("input");
                inGroup.append("svg:text").attr("class","red-ui-flow-port-label red-ui-flow-port-index").attr("x",20).attr("y",28).text(function(d,i){ return i+1});
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        if (hideLabel) {
                            ...
                            d.h = Math.max(6+24*labelParts.lines.length, (Math.max(d.outputs, d.inputs) || 0) * 15, 30);
                        }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        for(var portIndex = 0; portIndex < numInputs; portIndex++ ) {
                            var portGroup;
            ...
            link.each(function(d) {
                ...
                if (d.added || d===selected_link || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    var y1 = -((numOutputs-1)/2)*13 +13*sourcePort;
                    var y2 = -((numInputs-1)/2)*13 +13*targetPort;
